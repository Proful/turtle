...
pub struct RendererServer {
    ...
    task_handle: Option<RemoteHandle<io::Result<std::process::Output>>>,
}
...
impl RendererServer {
    ...
    pub async fn spawn() -> Result<(Self, ClientSender, ClientReceiver), ConnectionError> {
        ...
        let (child, child_handle) = child.wait_with_output().remote_handle();
        tokio::spawn(child);
...
impl Drop for RendererServer {
    fn drop(&mut self) {
        ...
        match self.runtime_handle.block_on(task_handle) {
            Ok(proc_status) => {
                if !proc_status.status.success() {
                    // Propagate error code from child process or exit with status code 1
                    process::exit(proc_status.status.code().unwrap_or(1));
                }
            }
            Err(err) => {
